    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          variant="ghost"
          className={cn(
            "text-foreground/70 hover:bg-secondary hover:text-foreground flex h-auto items-center gap-1.5 rounded-full border px-2 py-1 text-xs font-normal",
            className,
          )}
        >
          <CoinsIcon size={14} />
      ...
      <DropdownMenuContent side="bottom" align="end" className=" w-80">
        ...
        <DropdownMenuLabel className="hidden" >{t.tokenUsage.view}</DropdownMenuLabel>
        <DropdownMenuRadioGroup
        className="hidden"
          value={preset}
          onValueChange={(value) =>
        ...
        <DropdownMenuSeparator className="hidden"/>
        <div className="hidden text-muted-foreground px-2 py-2 text-xs leading-relaxed">
          {t.tokenUsage.note}
        </div>
      </DropdownMenuContent>
    </DropdownMenu>
