...
_PATENT_VERSION_RE_TEMPLATE = r"^{base}_v1\.(\d+){suffix}$"
...
def _next_minor_version(outputs_dir: Path, title: str, suffix: str) -> int:
    pattern = re.compile(_PATENT_VERSION_RE_TEMPLATE.format(base=re.escape(title), suffix=re.escape(suffix)))
    versions: list[int] = []
...
def finalize_patent_output_tool(
    ...
    `专利名称_v1.x.<ext>` in `/mnt/user-data/outputs`.
    ...
    try:
        ...
        minor = _next_minor_version(outputs_dir, normalized_title, suffix)
        target_name = f"{normalized_title}_v1.{minor}{suffix}"
